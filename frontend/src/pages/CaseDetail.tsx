...
Generated by SAR Tracking System`;
  ...
  const generateInitialSARRequest = async (caseData: SARCase) => {
    try {
      const response = await axios.get(`/reports/initial-sar/${caseData.id}`, {
        responseType: 'blob'
      });
      ...
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `${caseData.case_reference}-Initial-SAR-Request.pdf`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
      
      toast.success('Initial SAR Request generated successfully');
    } catch (error) {
      console.error('Error generating initial SAR request:', error);
      toast.error('Failed to generate initial SAR request');
    }
  };
