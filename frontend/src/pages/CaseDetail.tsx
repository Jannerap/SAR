...
const CaseDetail: React.FC = () => {
  ...
  const [showIcoModal, setShowIcoModal] = useState(false);
  const [icoDraft, setIcoDraft] = useState<string>('');
  const [icoForm, setIcoForm] = useState({
    escalation_date: new Date().toISOString().split('T')[0],
    escalation_method: 'Online Form',
    escalation_reason: 'Breach of SAR statutory response time',
    ico_reference: ''
  });
  ...
  const openIcoModal = async () => {
    if (!id) return;
    try {
      const response = await axios.get(`/sar/${id}/ico/draft`, { responseType: 'text' as any });
      setIcoDraft(typeof response.data === 'string' ? response.data : String(response.data));
      setShowIcoModal(true);
    } catch (error) {
      console.error('Error fetching ICO draft:', error);
      toast.error('Failed to load ICO draft');
    }
  };

  const handleEscalateToICO = async () => {
    if (!id) return;
    try {
      await axios.post(`/sar/${id}/ico/escalate`, {
        escalation_date: icoForm.escalation_date,
        escalation_method: icoForm.escalation_method,
        escalation_reason: icoForm.escalation_reason,
        ico_reference: icoForm.ico_reference || undefined
      });
      toast.success('ICO escalation created');
      setShowIcoModal(false);
      fetchCaseData();
    } catch (error) {
      console.error('Error escalating to ICO:', error);
      toast.error('Failed to escalate to ICO');
    }
  };

  const downloadIcoLetter = async (format: 'pdf' | 'txt') => {
    if (!id) return;
    try {
      const response = await axios.get(`/sar/${id}/ico/letter?format=${format}`, { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `${caseData?.case_reference}-ICO-Letter.${format}`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
      toast.success(`ICO letter downloaded (${format.toUpperCase()})`);
    } catch (error) {
      console.error('Error downloading ICO letter:', error);
      toast.error('Failed to download ICO letter');
    }
  };

  const downloadIcoBundle = async () => {
    if (!id) return;
    try {
      const response = await axios.get(`/sar/${id}/ico/bundle`, { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `ico-escalation-${caseData?.case_reference}.zip`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
      toast.success('ICO bundle downloaded');
    } catch (error) {
      console.error('Error downloading ICO bundle:', error);
      toast.error('Failed to download ICO bundle');
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          ...
          {isOverdue && !caseData.response_received && (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              ...
              <button 
                onClick={openIcoModal}
                className="w-full bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 px-4 py-2 rounded-lg transition-colors flex items-center justify-center"
              >
      ...
      {/* ICO Escalation Modal */}
      {showIcoModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-3xl mx-4">
            <h3 className="text-lg font-medium text-gray-900 mb-4">ICO Escalation</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Escalation Date</label>
                <input
                  type="date"
                  value={icoForm.escalation_date}
                  onChange={(e) => setIcoForm({...icoForm, escalation_date: e.target.value})}
                  className="w-full border border-gray-300 rounded-md px-3 py-2"
                />
              </div>
              <div>
                ...
                <select
                  value={icoForm.escalation_method}
                  onChange={(e) => setIcoForm({...icoForm, escalation_method: e.target.value})}
                  className="w-full border border-gray-300 rounded-md px-3 py-2"
                >
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1">ICO Reference (optional)</label>
                <input
                  type="text"
                  value={icoForm.ico_reference}
                  onChange={(e) => setIcoForm({...icoForm, ico_reference: e.target.value})}
                  className="w-full border border-gray-300 rounded-md px-3 py-2"
                  placeholder="ICO-XXXX..."
                />
              </div>
              <div className="md:col-span-2">
                ...
                <input
                  type="text"
                  value={icoForm.escalation_reason}
                  onChange={(e) => setIcoForm({...icoForm, escalation_reason: e.target.value})}
                  className="w-full border border-gray-300 rounded-md px-3 py-2"
                />
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1">Draft Complaint</label>
                <textarea
                  value={icoDraft}
                  onChange={(e) => setIcoDraft(e.target.value)}
                  rows={10}
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mt-4 gap-3">
              <div className="flex gap-3">
                <button onClick={() => downloadIcoLetter('pdf')} className="btn-secondary flex items-center">
                  <Download className="w-4 h-4 mr-2" /> Download ICO Letter (PDF)
                </button>
                <button onClick={() => downloadIcoLetter('txt')} className="btn-secondary flex items-center">
                  <Download className="w-4 h-4 mr-2" /> Download ICO Letter (TXT)
                </button>
                <button onClick={downloadIcoBundle} className="btn-secondary flex items-center">
                  <Download className="w-4 h-4 mr-2" /> Download ICO Bundle (ZIP)
                </button>
              </div>
              <div className="flex gap-3 justify-end">
                <button onClick={() => setShowIcoModal(false)} className="btn-secondary">Close</button>
                <button onClick={handleEscalateToICO} className="btn-primary">Create Escalation</button>
              </div>
            </div>
          </div>
        </div>
